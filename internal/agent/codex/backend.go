...
const DefaultImage = "steerlane/codex-agent:latest"
...
func NewBackend(logger *slog.Logger) (*Backend, error) {
	...
	return &Backend{
		logger: logger.With("agent_type", "codex"),
...
func (b *Backend) StartSession(ctx context.Context, opts agent.SessionOpts) error {
	...
	if b.started {
		b.mu.Unlock()
		return errors.New("codex: session already started")
	}
	b.started = true
	b.active = true
	b.mu.Unlock()

	b.logger.InfoContext(ctx, "codex session starting",
...
func (b *Backend) SendPrompt(ctx context.Context, prompt string) error {
	...
	if !started {
		return errors.New("codex: session not started")
	}
	if !active {
		return errors.New("codex: session not active")
	}

	b.logger.InfoContext(ctx, "sending prompt to codex agent", "prompt_len", len(prompt))
	return nil
}
...
func (b *Backend) Cancel(ctx context.Context) error {
	...
	b.active = false
	b.logger.InfoContext(ctx, "cancelling codex session")
	return nil
}
...
func RegisterDefault(registry *agent.Registry) {
	registry.Register(domainagent.TypeCodex, Factory())
}
