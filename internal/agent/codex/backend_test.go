...
func testCodexLogger() *slog.Logger {
	return slog.New(slog.NewTextHandler(io.Discard, nil))
}

func testCodexSessionOpts() agentpkg.SessionOpts {
...
func TestBackendLifecycle(t *testing.T) {
	...
	backend, err := NewBackend(testCodexLogger())
	require.NoError(t, err)

	require.ErrorContains(t, backend.SendPrompt(t.Context(), "continue"), "session not started")
	require.NoError(t, backend.StartSession(t.Context(), testCodexSessionOpts()))
...
func TestRegisterDefault(t *testing.T) {
	...
	assert.True(t, registry.Has(domainagent.TypeCodex))

	backend, err := registry.Create(domainagent.TypeCodex, testCodexLogger())
	require.NoError(t, err)
