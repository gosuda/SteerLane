...
func testGeminiLogger() *slog.Logger {
	return slog.New(slog.NewTextHandler(io.Discard, nil))
}

func testGeminiSessionOpts() agentpkg.SessionOpts {
...
func TestBackendLifecycle(t *testing.T) {
	...
	backend, err := NewBackend(testGeminiLogger())
	require.NoError(t, err)

	require.ErrorContains(t, backend.SendPrompt(t.Context(), "continue"), "session not started")
	require.NoError(t, backend.StartSession(t.Context(), testGeminiSessionOpts()))
...
func TestRegisterDefault(t *testing.T) {
	...
	backend, err := registry.Create(domainagent.TypeGemini, testGeminiLogger())
	require.NoError(t, err)
